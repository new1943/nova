...
use super::config::{TeamManager, Task, TaskStatus};
use super::mailbox::{Mailbox, MailMessage};
...
impl TeamTool {
    pub fn new(teams_dir: PathBuf) -> Self {
        Self { manager: TeamManager::new(teams_dir), mailbox: RwLock::new(Mailbox::new()) }
    }
}

#[async_trait]
impl ToolHandler for TeamTool {
    fn name(&self) -> &str { "team" }

    fn description(&self) -> &str {
        "Manage teams of agents. Create teams, add and assign tasks, and send messages between agent members."
    }
    ...
    fn input_schema(&self) -> Value {
        json!({
            "type": "object",
            "properties": {
                "action": { "type": "string", "enum": ["create_team", "list_teams", "add_task", "assign_task", "list_tasks", "send_message"] },
                "team": { "type": "string" },
    ...
    async fn execute(&self, args: Value, _ctx: &ToolContext) -> Result<String> {
        ...
        match action {
            ...
            "list_teams" => {
                ...
                if names.is_empty() { Ok("No teams exist.".to_string()) }
                else { Ok(format!("Teams:\n{}", names.join("\n"))) }
            }
            "add_task" => {
                let team = args.get("team").and_then(|v| v.as_str())
                    .ok_or_else(|| anyhow::anyhow!("Missing 'team' field"))?;
            ...
            "list_tasks" => {
                ...
                let t = self.manager.load(team)?
                    .ok_or_else(|| anyhow::anyhow!("Team '{}' not found", team))?;
                if t.tasks.is_empty() { return Ok("No tasks in this team.".to_string()); }
                let lines: Vec<String> = t.tasks.iter().map(|task| {
                    let owner = task.owner.as_deref().unwrap_or("(unassigned)");
                    format!("[{}] {:?} — {} (owner: {})", task.id, task.status, task.description, owner)
                }).collect();
                Ok(lines.join("\n"))
            }
            "send_message" => {
                ...
                let msg = MailMessage { from: "coordinator".to_string(), to: member.to_string(), content: content.to_string(), timestamp: Utc::now() };
