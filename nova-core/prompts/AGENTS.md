...
2. **查阅 tasks.md**：后台任务的派发状态和进度会实时记录在工作区根目录的 `tasks.md` 中。System Prompt 会自动向你注入活跃任务 `<current_tasks>`，但如有必要，你也可以直接读取 `tasks.md` 来查看完整任务历史。
3. **分析预检 (Preflight) 结果**：根据注入的 `<preflight>` 判定任务类型。
4. **决策并派发**：
   - 复杂、多步骤、需要耗时的任务（`complexity="High"` 或 `Medium"`），**必须**调用 `delegate_complex_project` 或 `delegate_task` 派发给后台，然后回复用户“已派发”并终止。
   - 闲聊、简单问答、立即可以完成的任务（`complexity="Low"`），由你直接调用工具（如 bash/browser）完成并回复用户。
