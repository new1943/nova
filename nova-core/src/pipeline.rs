...
use crate::preflight_types::{Complexity, PreFlightCheckResult};
...
impl TurnContext {
    ...
    /// 创建带平台信息的 TurnContext（Builder 模式）
...
impl fmt::Display for TurnContext {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "TurnContext(complexity={:?}, topic_shift={}, tools={}, injections={}, terminate={}, platform={})",
            self.complexity,
            self.topic_shift,
            ...
            self.platform.as_ref().map(|p| p.to_string()).unwrap_or("none".into()),
        )
    }
}
...
mod tests {
    ...
    #[test]
    fn test_turn_context_platform_defaults_to_none() {
        let ctx = TurnContext::new("hello".into(), vec![]);
        assert_eq!(ctx.platform, None);
    }

    #[test]
    fn test_with_platform_sets_discord() {
        let ctx = TurnContext::new("hello".into(), vec![])
            .with_platform(Platform::Discord);
        assert_eq!(ctx.platform, Some(Platform::Discord));
    }

    #[test]
    fn test_with_platform_sets_tui() {
        let ctx = TurnContext::new("hello".into(), vec![])
            .with_platform(Platform::Tui);
        assert_eq!(ctx.platform, Some(Platform::Tui));
    }

    #[tokio::test]
    async fn test_stage_works_with_none_platform() {
        // Verify that stages don't error when platform is None
        let mut pipeline = TurnPipeline::new();
        pipeline.add_stage(Box::new(DummyStage {
            name: "classify".into(),
            complexity: Complexity::High,
        ...
        // Pipeline should run without errors even with platform=None
        pipeline.run(&mut ctx).await.unwrap();
        assert_eq!(ctx.complexity, Complexity::High);
    }

    #[test]
    fn test_display_includes_platform_info() {
        let ctx = TurnContext::new("test".into(), vec![])
            .with_platform(Platform::Discord);
        let display = format!("{}", ctx);
        assert!(display.contains("platform=Discord"));

        let ctx_none = TurnContext::new("test".into(), vec![]);
        let display_none = format!("{}", ctx_none);
        assert!(display_none.contains("platform=none"));
    }
}
